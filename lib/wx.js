...
var wx = module.exports = {
  ...
  OAuth2: {
    qrAuthorizeUrl: function(appId, scope, state, redirectUrl) {
      return 'https://open.weixin.qq.com/connect/qrconnect?appid=' + appId + '&redirect_uri=' + redirectUrl + '&response_type=code&scope=' + scope + '&state=' + state + '#wechat_redirect';
    },
  ...
  Messaging: {
    sendTemplateMsg: function(accessToken, messageBody) {
      ...
      return promisify(request.post)(url, { body: JSON.stringify(messageBody) }).then(wx._parseResponse);
    }
  },
